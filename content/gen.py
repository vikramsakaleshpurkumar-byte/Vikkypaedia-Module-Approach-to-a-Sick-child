...
class Q:
    def __init__(self, stem, opts, correct, nudge, hint, why, wrong, teach, remed):
...
def _q_html(n, i, q):
    ...
    return '''    <div class="q" data-q="%d.%d" data-unit="%d" data-remed="%s">
...
def move_answer(n, qi, target):
    ...
    parts = _re.split(r"(?=<b>[A-D]</b> &mdash;)", q.wrong)
    head, items = parts[0], sorted(parts[1:], key=lambda s: s[3])
